...
export function EmployerDashboard() {
  const { data: opportunities, isLoading } = useEmployerOpportunities();
  const primaryOpportunity = opportunities?.[0];
  const { data: analytics } = useOpportunityAnalytics(primaryOpportunity?.id ?? "opp-backend-nova");

  if (isLoading) return <LoadingState label="Loading employer workspace..." />;

  const stats = [
    { label: "Total views", value: analytics?.uniqueViews.toLocaleString() ?? "9,270" },
    { label: "Acceptance rate", value: Math.round((analytics?.acceptanceRate ?? 0.197) * 100), suffix: "%" },
    { label: "Avg decision", value: formatMs(analytics?.averageDecisionTimeMs ?? 8400) },
    { label: "Submissions", value: analytics?.submissionsCount ?? 124 },
        ...
        <article className="rounded-2xl border border-[var(--employer-line)] bg-[var(--employer-surface)] p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-black">{primaryOpportunity?.roleTitle ?? "Backend Engineer"}</h2>
            </div>
            <Badge>{Math.round((analytics?.completionRate ?? 0.46) * 100)}% completion</Badge>
          </div>
          <div className="mt-6 h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={analytics?.decisionTimeDistribution ?? []}>
                <defs>
